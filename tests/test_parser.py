...
class TestParser(unittest.TestCase):
    def test_parse1(self):
        data = b"\x45\x00" \
            b"\x00\x29\x6f\x94\x40\x00\x80\x06\xe3\xd7\xc0\xa8" \
            b"\x00\x67\x40\xe9\xa5\x6a\xc6\x1b\x01\xbb\x78\x25" \
            b"\xc3\x2f\x0d\x8c\xb3\x0b\x50\x10\x01\x00\x43\xad" \
            b"\x00\x00\x00"
        packet = TCPPacketParser.TCPPacketParser(data)
        self.assertEqual('192.168.0.103', packet.source_ip, "Wrong source ip")
        self.assertEqual(
            '64.233.165.106', packet.dest_ip, "Wrong destination ip")
    ...
    def test_filter(self):
        packet1 = TCPPacketParser.TCPPacketParser(
            b'asdadsadadadadsasdasdasdasdadadsasdadasdadsasdasdasdasdasdad')
        packet1.source_ip = '1.1.1.1'
        packet2 = TCPPacketParser.TCPPacketParser(
            b'asdasdasdasdasdasdasdasdasdasdasdasdasdasdadsasdasdasdasdasd')
        packet2.source_ip = '2.2.2.2'

        addr_list = [('1.1.1.1', 50), ('1.2.3.4', 60)]
        self.assertTrue(
            packet1.filter_by_addr_list(addr_list),
            "Wrong filter for first ip")
        self.assertFalse(
            packet2.filter_by_addr_list(addr_list),
            "Wrong filter for second ip")
